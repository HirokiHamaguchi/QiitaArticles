...
def dp_func_profile_ProblemC(profile1: Profile, profile2: Profile, unstable=30) -> tuple:
    ...
    dp = [[0 for _ in range(len(p2)+1)]
          for _ in range(len(p1)+1)]
    rev = [[0 for _ in range(len(p2)+1)]  # 上記参照
           for _ in range(len(p1)+1)]
...
def solve(M, Ss) -> list:
    ...
    if score_after_progressive_alignment_generated_by_old_version >\
        ...
        score_after_progressive_alignment =\
            score_after_progressive_alignment_generated_by_old_version
        sys.stderr.write(
            "Using \"OLD\" version method for creating the start point.\n")
    else:
        sys.stderr.write(
            "Using \"NEW\" version method for creating the start point.\n")
