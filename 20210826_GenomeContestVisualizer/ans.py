...
def similarity(a: int, b: int, same, different, nan, nan_nan) -> int:
    if a == b:
        if a != NAN:
            return same
        else:
...
def similarity_profile(a: list, b: list, same, different, nan, nan_nan) -> float:
...
def vis(
    output_DNAs, seed: int = -1, color_mode: str = "SOFT", precise: bool = True
) -> int:
    """qiitaにあげたやつ"""
    M = len(output_DNAs)
...
def dp_func(DNA1: list, DNA2: list, unstable=30) -> tuple:
    ...
    rev = [[0 for _ in range(len(DNA2) + 1)] for _ in range(len(DNA1) + 1)]
    dp[0][0] = 0
...
def dp_func_profile(
    profile1: Profile, profile2: Profile, same, different, nan, nan_nan, unstable=30
) -> tuple:
    """
       A   C   G   T   NAN
...
def _dp_fpPC_calc_score(M, cnt1, cnt2):
    ...
    a_plus_b.sort(reverse=True)
    # print(a_plus_b, M)
    # time.sleep(0.05)
    # m1, m2, m3 = a_plus_b[0], a_plus_b[1], a_plus_b[2]
    # ret = (m1-M)*M*M+m2*M+m3
    ret = (a_plus_b[0] - M) * (M + 1) + a_plus_b[1]
...
def dp_func_profile_ProblemC(
    profile1: Profile, profile2: Profile, unstable=30
) -> tuple:
    ...
    rev = [[0 for _ in range(len(p2) + 1)] for _ in range(len(p1) + 1)]  # 上記参照
...
def iterative_refinement(
    M, DNAs, raw_DNAs, same, different, nan, nan_nan, use_p3_score_func: bool
):
    """
    先程挙げたpdfからの引用：
    ...
    for j in range(M):
        ...
        profile1 = Profile(DNAs=[raw_DNAs[j]], integrated_with_cnt=None)  # 注目DNA
        profile2 = Profile(
...
def solve(M, Ss) -> list:
    ...
    DNAs_generated_by_old_version = remove_unnecessary_NAN(
        M, DNAs_generated_by_old_version
    )
    score_after_progressive_alignment_generated_by_old_version = calc_score(
        len(DNAs_generated_by_old_version[0]),
        M,
    ...
    if (
        ...
        sys.stderr.write('Using "OLD" version method for creating the start point.\n')
    else:
        sys.stderr.write('Using "NEW" version method for creating the start point.\n')
    ...
    DNAs = annealing(M, DNAs, ANNEALING_TL=9.0, START_TEMP=None, END_TEMP=None)
...
if __name__ == "__main__":
    ...
    # main(seed=6, l_or_s="s", is_local=True)
    # exit()
    # main(l_or_s="l")
