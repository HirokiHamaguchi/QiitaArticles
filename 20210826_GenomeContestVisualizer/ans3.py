...
def remove_unnecessary_NAN(M, DNAs, char_idx=None):
...
def solve(M, Ss) -> list:
    ...
    #                             nan=-1,
    #                             nan_nan=-1,
    #                             use_p3_score_func=False)
    # score_after_iterative_refinement1 = calc_score(
    #     len(DNAs[0]), M, DNAs, precise=False)
    # score_after_iterative_refinement1 = -1
    # sys.stderr.write("score(after iterative_refinement1):"
    #                  + f"{score_after_iterative_refinement1}\n")
    # DNAs = remove_unnecessary_NAN(M, DNAs)

    # 揃うのを最優先するやつ これは予備を作らない。
    # DNAs = iterative_refinement(M, DNAs,
    #                             same=100,
    #                             different=0,
    #                             nan=-1,
    #                             nan_nan=-1,
    #                             use_p3_score_func=False)
    # score_after_iterative_refinement2 = calc_score(
    #     len(DNAs[0]), M, DNAs, precise=False)
    # score_after_iterative_refinement2 = -1
    # sys.stderr.write("score(after iterative_refinement2):"
    #                  + f"{score_after_iterative_refinement2}\n")
    # DNAs = remove_unnecessary_NAN(M, DNAs)
...
def main(seed=1, l_or_s="s", is_local=True) -> int:
    ...
    if not is_local:
        M = int(input())
        Ss = [list(map(parse_for_input, input())) for _ in range(M)]
    else:
...
if __name__ == "__main__":
    ...
    main(seed=10, l_or_s="s", is_local=True)
    exit()
    # main(l_or_s="l")
