...
def calc_score(N: int, M: int, DNAs: list):
    ...
    if 0 <= M <= 10:
        ...
        raise AssertionError(f"M:{M} このAssertionErrorは外しても問題ありませんが、"
...
def vis_difference(M: int, score1: int, score2: int, DNAs1: list, DNAs2: list, difference: list, color_mode=None):
    N = len(DNAs1[0])
...
def main(args):
    ...
    if len(args) == 3 or len(args) == 4:
        ...
        with open(path1) as f:
            M_ = int(f.readline())
            DNAs1 = [list(map(parse_for_input, f.readline().rstrip("\n")))
                     for _ in range(M_)]
        with open(path2) as f:
            M = int(f.readline())
            assert M_ == M, "二つのファイルのＭが異なります"
            DNAs2 = [list(map(parse_for_input, f.readline().rstrip("\n")))
                     for _ in range(M)]
