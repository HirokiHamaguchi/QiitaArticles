...
まず、「数値型の参照渡し(**のようなこと**)」と先程書きましたが、お察しの通り、これはあまり正確な言い方ではありません。そして、その正確な言い方が何であるかはこの記事の範囲を逸脱するので、誤魔化すことにします。以下の文章も含めてそれは誤魔化します。(あまり胸を張って誤魔化すなどと言うものではありませんが……)

(少しだけ言い訳めいた主張を述べると、「JavaScript 値渡し 参照渡し」などと検索すれば、ご存じの通りこの話題に関する記事が大量に出てきますが、「**参照渡しも値渡しもある**」「**実は値渡ししかない**」「**実は参照渡しも値渡しも存在しない**」「**実は共有渡し**」「**実は共有渡しじゃない**」などと、もしかしてそれぞれ違う言語のお話をされていますか? みたいな悲惨な様相を呈しています。そして往々にしてこの手の記事は正確な出典や参考文献が無い、もしくは曖昧で、個人の帰納的な推測から語られがちな印象があります。一応[MDN](https://developer.mozilla.org/ja/docs/Web/JavaScript/Guide/Functions) には「値渡し」という語が存在するようです([追記]コメントでこの点に関する言及も頂きました)。 尤も、今回の記事の範囲内に関しては、これを誤魔化してもクリティカルな影響は無いだろうと判断したため、このような措置を取らせて頂きました)
...
さて、私はこれを実現させようと、以下のようなコードを書きました。（注記: 以下は **あまり良くない書き方** です）
...
ただ、これら等価性比較についてその **厳密な** 仕様はかなり複雑です。
